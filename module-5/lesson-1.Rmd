...
```{r first-plot-scatterplot-rendered, echo=FALSE, fig.height=3.5, fig.width=11}
library(gridExtra)
...
```{r scatterplot-by-store, echo=TRUE, fig.height=3.5, fig.width=6}
df %>%
...
```{r scatterplot-specifying-all-points-as-blue, fig.height=3.5, fig.width=6}
ggplot(df, aes(x = quantity, y = sales_value)) +
...
```{r scatterplot-with-smoothed-line, fig.height=3.5, fig.width=6}
...
```{r scatterplot-with-smoothed-line-and-specified-color, fig.height=3.5, fig.width=6}
ggplot(df, aes(x = quantity, y = sales_value)) +
  geom_point(color = "blue") +
...
```{r statistical-transformation, fig.height=4}
ggplot(df, aes(x = household_size)) +
  geom_bar() 
```
...
```{r counts-bar-plot, fig.height=4}
...
```{r stat-transformation-mean, fig.height=4}
df %>%
...
```{r position-adjustments, fig.width=6}
...
```{r flip-axes, fig.height=4, fig.width=4}
...
```{r bar-y-axis-percentage-format, fig.width=6, fig.height=4}
ggplot(df, aes(x = household_size, fill = brand)) + 
  geom_bar(position = "fill") +
...
```{r color-brewer-palette-example2, echo=FALSE, fig.width=10, fig.height=4}
...
```{r coordinate2, echo=FALSE, fig.width=10, fig.height=4}
