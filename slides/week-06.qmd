...
Open the [Week 6 companion notebook](https://colab.research.google.com/github/bradleyboehmke/uc-bana-7025/blob/main/notebooks/tuesday-your-turn/week-06-lecture.ipynb){target="_blank"} in Google Colab and follow along as we work through today's examples.
:::
...
## Challenge: Grocery Loyalty Classifier {.smaller}

**Your turn — write this one from scratch.**

**Business rules:**

| Monthly Spend | Loyalty Status | Discount |
|--------------|---------------|---------|
| ≥ $500 | Gold | 10% |
| ≥ $200 | Silver | 5% |
| ≥ $50 | Bronze | 2% |
...
1. Sets a `monthly_spend` variable
2. Assigns `loyalty_status` and `discount` using `if/elif/else`
3. Prints: `f"${monthly_spend}/month → {loyalty_status} ({discount:.0%} off)"`

**Test with:** $650 → Gold, $275 → Silver, $75 → Bronze, $30 → No Status
:::
::: {.column width="25%"}
<center>

<div id="loyaltyChallenge"></div>
<script>
    document.addEventListener("DOMContentLoaded", function () {
        initializeTimer("loyaltyChallenge", 300, "slide");
    });
</script>
</center>
:::
...
## Solution: Grocery Loyalty Classifier {.smaller}
...
if monthly_spend >= 500:
    ...
    loyalty_status = "No Status"
    discount = 0.0

print(f"${monthly_spend}/month → {loyalty_status} ({discount:.0%} off)")
```
...
## Challenge: Flag Transactions with `np.where()` {.smaller}

**Scenario:** Use the Complete Journey transactions data to classify purchases.
...
:::: {.columns}
::: {.column width="65%"}
**Your task:** Use `np.where()` to add two new columns:

1. `'value_tier'` — `'premium'` if `sales_value > 10`, else `'standard'`
2. `'disc_flag'` — `'discounted'` if `retail_disc > 0`, else `'full price'`

Then use `.value_counts()` on each to see the split.
:::
::: {.column width="35%"}
<center>

<div id="npwhereChallenge"></div>
<script>
    document.addEventListener("DOMContentLoaded", function () {
        initializeTimer("npwhereChallenge", 300, "slide");
    });
</script>
</center>
:::
...
## Solution: Flag Transactions with `np.where()` {.smaller}
...
df['value_tier'] = np.where(df['sales_value'] > 10, 'premium', 'standard')
...
```{python}
df['disc_flag'] = np.where(df['retail_disc'] > 0, 'discounted', 'full price')
df['disc_flag'].value_counts()
```
...
## Challenge: Loyalty Classifier — Now With a Loop {.smaller}

**Earlier** you classified one household's loyalty status with `if/elif/else`.

**Now** use a `for` loop to classify all of them at once.
...
**Your task:** Loop over `households` and for each spending amount, print:

`"$X/month → Status (Y% off)"`

using the same Gold / Silver / Bronze / No Status rules from before.
:::
::: {.column width="35%"}
<center>

<div id="loyaltyLoopChallenge"></div>
<script>
    document.addEventListener("DOMContentLoaded", function () {
        initializeTimer("loyaltyLoopChallenge", 300, "slide");
    });
</script>
</center>
:::
...
## Solution: Loyalty Classifier — Now With a Loop {.smaller}

```{python}
households = [650, 275, 75, 30, 520, 190]

for spend in households:
    if spend >= 500:
        status, disc = "Gold", 0.10
    elif spend >= 200:
        status, disc = "Silver", 0.05
    elif spend >= 50:
        status, disc = "Bronze", 0.02
    else:
        status, disc = "No Status", 0.0
    print(f"${spend}/month → {status} ({disc:.0%} off)")
```
...
- **DO NOT create your own group** outside the Canvas list
...
- **File naming:** `YYYY_BANA7025_groupXX_finalproject.html`
...
Most of you should have a signed-off business problem and a plan for the analyses needed to answer it. **Use Thursday's lab to execute that work** — I'll be circulating to provide feedback as you go.

**If you don't yet have sign-off or an analysis plan, that's your first priority Thursday:**

- Get your business question approved by me or the TA
- Map out the specific analyses needed to answer it (what data, what metric, what visualization?)

**Once those are in place, start executing:**

- Work through your analyses in priority order
- Ask for feedback early — don't wait until something is finished
- Aim to have at least one complete analysis by end of lab

::: {.callout-important}
**Come prepared:** Laptop ready, proposal open, and a clear sense of what you're working on Thursday.
:::
...
- ✅ Business question finalized & approved
- 🔄 Analyses mapped out (Tuesday/Thursday)
- 🔄 Begin executing analyses (Thursday lab)
:::
::: {.column}
**Week 7 — Final Push:**
...
- 📝 Write report
- 🎥 Record presentation
- 📤 **Submit by end of Week 7**
:::
::::

**Bottom line:** Your foundation is set — Week 6 is about locking in the plan and getting work done.
