...
At the end of every lesson, and indeed at the end (or beginning) of any notebook you make, we should include information about the computing environment including the version numbers of all packages we use. We can do that with the following. `sessioninfo()` will provide information on the operating system, version of R along with any specified packages. Here, we specify `pkgs = 'attached'` which means it will only list those packages that have been attached to the R search path with `library(pkg_name)`.
