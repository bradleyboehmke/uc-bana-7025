...
We can apply several of these comparison operators in the `filter` function.  For example, we can filter for transactions for a particular store ID and household ID:
...
```{block, type = "note"}
A comma between comparison operators acts just like and `&` operator.  So `filter(data, variable1 == value, variable2 == value)` is the same as `filter(data, variable1 == value & variable2 == value)`
```
...
```{r summarize-functions, echo=FALSE}
